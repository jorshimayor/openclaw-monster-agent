...
/**
 * Streams pipeline events. Renders an explicit empty state rather than a
 * fabricated "log panel initialized" entry — that placeholder was counted in
 * the "LIVE · N ENTRIES" footer, so an idle panel claimed one event it never
 * received.
 */
export function LogPanel({ entries }: LogPanelProps) {
  ...
  return (
    ...
    >
      {data.length === 0 ? (
        <div className="h-full flex items-center justify-center text-matrix-dim/70 tracking-widest text-[11px]">
          {hydrated ? "NO EVENTS YET · AWAITING PIPELINE STREAM" : "CONNECTING…"}
        </div>
      ) : (
        <>
          <div className="space-y-0.5">
            {data.map((l, i) => (
              <div key={i} className="flex gap-3 whitespace-pre">
                <span className="text-matrix-dim/70 shrink-0 select-none">{l.time}</span>
          ...
          <div className="mt-3 flex items-center gap-2 text-matrix-dim/80">
            ...
            <span className="text-[10px] tracking-widest">
              LIVE · {data.length} {data.length === 1 ? "ENTRY" : "ENTRIES"}
            </span>
          </div>
        </>
      )}
